...
class TestCondaPackageUtil(unittest.TestCase):
    ...
    def test_lookup_package(self, mock_get_cache):
        mock_get_cache.return_value = {
            "packages": {
                "numpy-1.18.0": {"name": "numpy", "version": "1.18.0"}
            },
            "packages.conda": {}
        }
    ...
    @patch("superbom.utils.packageindexes.conda.condacache.CondaCache.channels", new_callable=PropertyMock)
    @patch("superbom.utils.packageindexes.conda.condacache.CondaCache.platforms", new_callable=PropertyMock)
    def test_retrieve_conda_package_info_success(self, mock_platforms, mock_channels, mock_get_cache):
    ...
    @patch("superbom.utils.packageindexes.conda.condadependencies.CondaCache.get_cache")
    @patch("superbom.utils.packageindexes.conda.condadependencies.CondaCache.channels", new_callable=PropertyMock)
    @patch("superbom.utils.packageindexes.conda.condadependencies.CondaCache.platforms", new_callable=PropertyMock)
    def test_retrieve_conda_package_info_no_license(self, mock_platforms, mock_channels, mock_get_cache):
        mock_channels.return_value = ["conda-forge"]
        mock_platforms.return_value = ["noarch"]
        mock_get_cache.return_value = {
            "packages": {
    ...
    @patch("superbom.utils.packageindexes.conda.condadependencies.CondaCache.channels", new_callable=PropertyMock)
    @patch("superbom.utils.packageindexes.conda.condadependencies.CondaCache.platforms", new_callable=PropertyMock)
    def test_retrieve_conda_package_info_not_found(self, mock_platforms, mock_channels, mock_get_cache):
        mock_channels.return_value = ["conda-forge"]
        mock_platforms.return_value = ["noarch"]
